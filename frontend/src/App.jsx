...
export default function App() {
  ...
  return (
    <div className="gradient-bg h-screen w-screen flex flex-col justify-between p-6 overflow-hidden">
      ...
      <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-6 overflow-hidden mb-4 min-h-0">
        ...
        <div className="md:col-span-1 flex flex-col h-full overflow-hidden min-h-0 space-y-4">
          ...
          <div className="flex-grow flex flex-col justify-between overflow-hidden min-h-0 space-y-4">
            ...
            {/* How it Works - always visible inside the right column bottom */}
            <section className="glass-panel p-4 space-y-2 shrink-0">
              <h3 className="text-xs uppercase tracking-wider font-extrabold text-teal-400">How it works</h3>
              <p className="text-[10px] text-slate-400 leading-normal">
                This detector pre-processes text (removing URLs, symbols, HTML, numbers) and transforms it into numerical vectors using a <strong>TF-IDF Vectorizer</strong> (50k features). A <strong>Logistic Regression</strong> model classifies the patterns, achieving a verified <strong>99.39% accuracy</strong> on the ISOT dataset.
              </p>
            </section>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="flex items-center justify-between border-t border-slate-800 pt-3 text-[10px] text-slate-600 shrink-0">
        <p>&copy; {new Date().getFullYear()} Veritas AI. Professional Portfolio Representation.</p>
        <p>Trained on ~44,000 articles</p>
      </footer>
    </div>
  );
}
